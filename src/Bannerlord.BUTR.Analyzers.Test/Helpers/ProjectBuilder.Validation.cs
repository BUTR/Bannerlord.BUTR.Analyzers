...
namespace TestHelper
{
    public sealed partial class ProjectBuilder
    {
        ...
        private Task<Project> CreateProject()
        {
            ...
            var fileExt = ".cs";
            var testProjectName = "Bannerlord.HarmonyAnalyzer.Test";
        ...
        private static void VerifyDiagnosticLocation(DiagnosticAnalyzer analyzer, Diagnostic diagnostic, Location actual, in DiagnosticResultLocation expected)
        {
            ...
            Assert.IsTrue(string.Equals(actualSpan.Path, expected.Path, StringComparison.Ordinal) || (actualSpan.Path != null && actualSpan.Path.StartsWith("Test", StringComparison.Ordinal) && expected.Path.EndsWith(".cs", StringComparison.Ordinal)),
